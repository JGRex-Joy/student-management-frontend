...
export function Dashboard() {
  ...
  const stats = [
    { label: 'Students', value: students.data?.totalElements ?? '—', sub: 'total registered', symbol: '◉', color: '#2d6a4f', bg: 'rgba(45,106,79,0.06)' },
    { label: 'Courses',  value: courses.data?.totalElements ?? '—',  sub: 'active courses',   symbol: '◎', color: '#1e4d8c', bg: 'rgba(30,77,140,0.06)' },
    { label: 'Enrolled', value: enrolled.data?.totalElements ?? '—', sub: 'students in courses', symbol: '◈', color: '#7c3d12', bg: 'rgba(124,61,18,0.06)' },
    {
      label: 'Coverage',
      value: (students.data?.totalElements && enrolled.data?.totalElements)
        ...
        : '—',
      sub: 'enrollment rate', symbol: '◐', color: '#5b2d8e', bg: 'rgba(91,45,142,0.06)',
    },
  ];
      ...
      <div className="dash-bottom">
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontFamily: 'var(--font-display)', fontWeight: 600, fontSize: 14, color: 'var(--text)', letterSpacing: '-0.02em' }}>Getting Started</div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-faint)', marginTop: 2 }}>how to use this system</div>
            </div>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 18, color: 'var(--cream-deeper)' }}>◈</span>
          </div>
          <div style={{ padding: '6px 20px 18px' }}>
            {steps.map(({ num, title, desc }) => (
              <div key={num} style={{ display: 'flex', gap: 16, padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-faint)', letterSpacing: '0.05em', paddingTop: 2, flexShrink: 0, width: 20 }}>{num}</div>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em', marginBottom: 2 }}>{title}</div>
                  <div style={{ fontSize: 11, color: 'var(--ink-muted)', fontFamily: 'var(--font-mono)', lineHeight: 1.5 }}>{desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="dash-right-col">
          ...
          <div style={{ background: 'var(--ink-2)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden', flex: 1 }}>
            ...
            <div style={{ padding: '6px 20px 18px' }}>
              {[
                'Duplicate course codes are blocked automatically.',
                'Deleting a student removes all their enrollments too.',
                'A student can be enrolled in multiple courses at once.',
                'Inactive students and courses are hidden from lists.',
                'Enrollment skips courses the student already has.',
              ].map((tip, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.05)', alignItems: 'flex-start' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'rgba(247,244,239,0.25)', flexShrink: 0, paddingTop: 1 }}>→</span>
                  <span style={{ fontSize: 12, color: 'rgba(247,244,239,0.55)', fontFamily: 'var(--font-mono)', lineHeight: 1.55 }}>{tip}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: 'var(--white)', borderRadius: 8, border: '1px solid var(--border)', padding: '14px 20px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {['Spring Boot 3', 'JPA / Hibernate', 'MySQL', 'BCrypt Auth', 'React 18', 'Vite'].map(tag => (
                <span key={tag} style={{ fontFamily: 'var(--font-mono)', fontSize: 10, border: '1px solid var(--border)', borderRadius: 4, padding: '4px 9px', color: 'var(--ink-muted)', letterSpacing: '0.02em' }}>{tag}</span>
              ))}
            </div>
          </div>
        </div>
...
function StatCard({ label, value, sub, symbol, color }) {
  return (
    <div style={{
      background: 'var(--white)', borderRadius: 8,
      ...
      <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: color, opacity: 0.35, borderRadius: '8px 8px 0 0' }} />
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 13, color: color, opacity: 0.4 }}>{symbol}</span>
      </div>
      <div style={{ fontFamily: 'var(--font-display)', fontSize: 30, fontWeight: 700, color: 'var(--ink)', lineHeight: 1, letterSpacing: '-0.04em', marginBottom: 6 }}>{value}</div>
      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--ink-faint)', letterSpacing: '0.04em' }}>{sub}</div>
    </div>
  );
}